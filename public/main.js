import { runEpisode } from './core/engine/episodeRunner.js';
...
(async () => {
    ...
    loadBtn.addEventListener('click', () => {
        const data = load(0);
        if (data && data.episodeId === ep.episodeId) {
            runEpisode(ep, container, data.nodeId, data.resources);
        }
    });
    menu.append(saveBtn, loadBtn);
    document.body.prepend(menu);
    await runEpisode(ep, container);
})();
